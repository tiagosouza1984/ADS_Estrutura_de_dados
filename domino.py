...
def peca_valida(peca):
        if type(peca.direita) != str and type(peca.direita) != float and type(peca.esquerda) != str and type(peca.esquerda) !=float:
            if peca.direita > 0 and peca.direita <= 6 and peca.esquerda >0 and peca.esquerda <= 6:
                return peca
...
def encaixa_de_algum_jeito(peca1,peca2):
    if peca1.direita == peca2.esquerda or peca1.direita == peca2.direita:
        return True
    elif peca1.esquerda == peca2.esquerda or peca1.esquerda == peca2.direita:
        return True
    else:
        return False
...
def encaixa_girando_so_a_segunda(peca1,peca2):
    if peca1.direita == peca2.esquerda:
        return True
    elif peca1.direita == peca2.direita:
        peca2.inverte()
        return True
    else:
        return False
...
def ciclo_valido(jogo):
    flag = False
    for num,peca in enumerate(jogo):
        if num != 0 and num < len(jogo)-1:
            if peca.esquerda == jogo[num-1].direita:
                flag = True
            else:
                flag = False
        elif num == len(jogo)-1:
            if peca.esquerda == jogo[num-1].direita and peca.direita == jogo[0].esquerda:
                flag = True
            else:
                flag = False
    return flag
...
def jogador(jogo,peca):
    resp = ""
    if peca.esquerda == jogo[len(jogo)-1].direita:
        resp = "frente"
    elif peca.esquerda == jogo[0].esquerda:
        peca.inverte()
        resp = "atras"
    elif peca.direita == jogo[len(jogo)-1].direita:
        peca.inverte()
        resp = "frente"
    elif peca.direita == jogo[0].esquerda:
        resp = "atras"
    else:
        resp = False
    return resp
...
def no_comeco(jogo,peca):
    novo_jogo = ""
    game_valido = jogo_valido(jogo)
    if game_valido == True:
        novo_jogo = jogo.insert(0, peca)
    return novo_jogo
...
def jogo_grande(pecas):
    ...
    for x in pecas:
        if len(jogo_grande) == 0:
            jogo_grande.insert(0, x)
        else:
            if x.esquerda == jogo_grande[len(jogo_grande)-1].direita:
                jogo_grande.append(x)
            elif x.esquerda == jogo_grande[0].esquerda:
                x.inverte()
                jogo_grande.insert(0,x)
            elif x.direita == jogo_grande[len(jogo_grande)-1].direita:
                x.inverte()
                jogo_grande.append(x)
            elif x.direita == jogo_grande[0].esquerda:
                jogo_grande.insert(0,x)
            else: pecas_sobrando.append(x)              
