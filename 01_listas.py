...
def cria_naipe(naipe):
    carta = []
    carta_num= ['A',str(2),str(3),str(4),str(5),str(6),str(7),str(8),str(9),str(10),'J','Q','K']
    for x in carta_num:
        carta.append(x+naipe)
    return carta
...
def cria_baralho():
    ...
    cria_naipes = {}
    for x in naipe:
        cria_naipes[x] = cria_naipe(x)
        for naipe in cria_naipes[x]:
            baralho.append(naipe)
    return baralho    
...
def  liquidacao(precos,descontos):
    precos_finais=[]
    for posicao, valor in enumerate(precos):
        precos_finais.append(valor-descontos[posicao])
