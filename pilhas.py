...
def menos_o_d(texto):
    ...
    for letra in texto:
        if letra != 'd':
            pilha_letras.append(letra)
            fotografa(pilha_letras)
        if letra == 'd' and len(pilha_letras) !=0:
            pilha_letras.pop()
            fotografa(pilha_letras)
    return pilha_letras
'''
...
def encaixa(abre, fecha):
    lista_abrefecha =[abre,fecha]
    if lista_abrefecha == ['(',')']:
        return True
    elif lista_abrefecha == ['[',']']:
        return True
    elif lista_abrefecha == ['{','}']:
        return True
    else:
...
def balanceada(string):
    ...
    max_len = 0
    for char in string:
        if len(lista_balance) == 0:
            lista_balance.append(char)
        elif len(lista_balance) !=0:
            if lista_balance[len(lista_balance)-1]+char == '()':
                lista_balance.pop()
            elif lista_balance[len(lista_balance)-1]+char == '[]':
                lista_balance.pop()
            elif lista_balance[len(lista_balance)-1]+char == '{}':
                lista_balance.pop()
            elif lista_balance[len(lista_balance)-1]+char == '<>':
                ...
                if char in ['(',')','[',']','{','}','<','>']:
                    lista_balance.append(char)
        if len(lista_balance) > max_len:
            max_len = len(lista_balance)
    fotografa(max_len)
    if len(lista_balance) == 0:
        return True
    else:
